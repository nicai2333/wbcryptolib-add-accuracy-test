...
int test_entire_process_works(wbcrypto_sm2coop_context* client, wbcrypto_sm2coop_context* server, int thread_num) {
    int ret = 0;
    char plaintext_buf[] = "encryption standard";
    ...
    printf("%d - %d \n", sizeof(plaintext_buf) - 1, sizeof(ciphertext_buf));
    double begin, end;
    size_t count=1000;
    begin = omp_get_wtime();
    ...
    printf("enc - %d threads: run %d times, total time: %f s, per second do: %f times\n", \
    ...
    printf("dec - %d threads: run %d times, total time: %f s, per second do: %f times\n", \
                thread_num, count, 1.0*(end-begin), count/(end-begin));
...
//    ASSERT_SUCCESS(test_entire_process_works(&client, &server));
    test_entire_process_works(&client, &server, 1);
    test_entire_process_works(&client, &server, 2);
    test_entire_process_works(&client, &server, 4);
    test_entire_process_works(&client, &server, 8);
    test_entire_process_works(&client, &server, 12);
    test_entire_process_works(&client, &server, 16);
    test_entire_process_works(&client, &server, 32);
