...
size_t test_sm4_lut_gcm_crypt_loop(size_t size){
    ...
    wbcrypto_sm4_lut_gcm_encrypt_init(&gcm_ctx, &ctx, test_iv_enc, sizeof(test_iv_enc), test_aad, sizeof(test_aad));
    for (count = 0; run && count < 0xffffffffffffffff; count++)
    {
        wbcrypto_sm4_lut_gcm_encrypt(&gcm_ctx, test_input, size, test_output);
    }
...
size_t test_sm4_lut_ctr_crypt_loop(size_t size){
    ...
    for (count = 0; run && count < 0xffffffffffffffff; count++)
    {
        wbcrypto_sm4_lut_ctr_encrypt(&ctx, test_iv_enc, test_ecount_buf, &test_num, test_input, size, test_output);
    }
...
size_t test_sm4_lut_cbc_crypt_loop(size_t size){
    ...
    for (count = 0; run && count < 0xffffffffffffffff; count++)
    {
        wbcrypto_sm4_lut_cbc_encrypt(&ctx, test_iv_enc, test_input, size, test_output);
    }
...
{
    WBCRYPTO_ecb128_encrypt(IN, OUT, ilen, ctx, (WBCRYPTO_block128_f)wbcrypto_sm4_lut_encrypt);
    return 0;
}
...
{
    WBCRYPTO_gcm128_init(gcm_ctx, ctx, (WBCRYPTO_block128_f)wbcrypto_sm4_lut_encrypt);
    WBCRYPTO_gcm128_setiv(gcm_ctx, iv, ivlen);
    WBCRYPTO_gcm128_aad(gcm_ctx, aad, aadlen);
    return 0;
}

int wbcrypto_sm4_lut_cbc_encrypt(const wbcrypto_sm4_lut_context *ctx, unsigned char* iv, const unsigned char IN[], size_t ilen, unsigned char OUT[])
{
    WBCRYPTO_cbc128_encrypt(IN, OUT, ilen, ctx, iv, (WBCRYPTO_block128_f)wbcrypto_sm4_lut_encrypt);
    return 0;
}

int wbcrypto_sm4_lut_gcm_encrypt(WBCRYPTO_GCM128_CONTEXT* gcm_ctx, const unsigned char IN[], size_t ilen, unsigned char OUT[])
{
    return WBCRYPTO_gcm128_encrypt(gcm_ctx, IN, OUT, ilen);
}
int wbcrypto_sm4_lut_ctr_encrypt(const wbcrypto_sm4_lut_context *ctx, unsigned char* iv, unsigned char* ecount_buf, unsigned int* num, const unsigned char IN[], size_t ilen, unsigned char OUT[])
{
    WBCRYPTO_ctr128_encrypt(IN, OUT, ilen, ctx, iv, ecount_buf, num, (WBCRYPTO_block128_f)wbcrypto_sm4_lut_encrypt);
    return 0;
}
...
void sm4_lut_accuracy_test()
{
    ...
    wbcrypto_sm4_lut_context *ctx=NULL;
    ctx = (wbcrypto_sm4_lut_context*)malloc(sizeof(wbcrypto_sm4_lut_context));
    for (int i=0;i<32;i++){
        ctx->rk[i]=0x00000000;
    }
    ...
    printf("SM4_LUT 1 block Accuracy Test.:\n");
    wbcrypto_sm4_lut_encrypt(pt_vector,output,ctx);
    printf("cipher text: \n");
    dump_hex(output,16);
    if(memcmp(ct_vector,output,16)==0)
        printf("SM4_LUT 1 block encrypt Test passed!\n");
    printf("SM4_LUT 1 block Accuracy Test end!\n\n");
