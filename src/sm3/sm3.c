...
void wbcrypto_sm3_init(wbcrypto_sm3_context* ctx) {
	memset(ctx, 0, sizeof(wbcrypto_sm3_context));
}

void wbcrypto_sm3_free(wbcrypto_sm3_context* ctx) {
	if (ctx == NULL)
		return;
	mbedtls_zeroize(ctx, sizeof(wbcrypto_sm3_context));
}
...
int wbcrypto_sm3_update(wbcrypto_sm3_context* ctx, const unsigned char* input, size_t ilen) {
	if (ctx->total[0]) {
		...
		if (ilen < left) {
			...
			wbcrypto_sm3_process(ctx, ctx->buffer);
			ctx->total[1]++;
			input += left;
			ilen -= left;
		}
	...
	while (ilen >= SM3_BLOCK_SIZE) {
		wbcrypto_sm3_process(ctx, input);
		ctx->total[1]++;
...
int wbcrypto_sm3(const unsigned char* msg, size_t msglen, unsigned char dgst[SM3_DIGEST_LENGTH]) {
	int ret = 0;
	wbcrypto_sm3_context ctx;
	wbcrypto_sm3_init(&ctx);
	MBEDTLS_MPI_CHK(wbcrypto_sm3_starts(&ctx));
	MBEDTLS_MPI_CHK(wbcrypto_sm3_update(&ctx, msg, msglen));
	MBEDTLS_MPI_CHK(wbcrypto_sm3_finish(&ctx, dgst));
cleanup:
