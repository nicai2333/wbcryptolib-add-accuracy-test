...
#  if !defined(OPENSSL_SMALL_FOOTPRINT)
/*
 * Streamed gcm_mult_4bit, see WBCRYPTO_gcm128_[en|de]crypt for
...
//                    ctx->Yi.d[3] = ctr;
//                while (len--) {
//                    ctx->Xi.c[n] ^= out[n] = in[n] ^ ct b      n  x->EKi.c[n];
//                    ++n;
//                }
...
{
#if defined(OPENSSL_SMALL_FOOTPRINT)
    return WBCRYPTO_gcm128_encrypt(ctx, in, out, len);
#else
    const union {
        long one;
        char little;
...
{
#if defined(OPENSSL_SMALL_FOOTPRINT)
    return WBCRYPTO_gcm128_decrypt(ctx, in, out, len);
#else
    const union {
        long one;
        char little;
...
{
    WBCRYPTO_gcm128_finish(ctx, NULL, 0);
    memcpy(tag, ctx->Xi.c,
...
{
    WBCRYPTO_GCM128_CONTEXT *ret;

    if ((ret = OPENSSL_malloc(sizeof(*ret))) != NULL)
        WBCRYPTO_gcm128_init(ret, key, block);
...
# define TEST_CASE(n)    do {                                    \
        ...
        WBCRYPTO_gcm128_init(&ctx,&key,(WBCRYPTO_block128_f)AES_encrypt);  \
        WBCRYPTO_gcm128_setiv(&ctx,IV##n,sizeof(IV##n));          \
        memset(out,0,sizeof(out));                              \
        if (A##n) WBCRYPTO_gcm128_aad(&ctx,A##n,sizeof(A##n));    \
        if (P##n) WBCRYPTO_gcm128_encrypt(&ctx,P##n,out,sizeof(out));     \
        if (WBCRYPTO_gcm128_finish(&ctx,T##n,16) ||               \
            (C##n && memcmp(out,C##n,sizeof(out))))             \
                ret++, printf ("encrypt test#%d failed.\n",n);  \
        WBCRYPTO_gcm128_setiv(&ctx,IV##n,sizeof(IV##n));          \
        memset(out,0,sizeof(out));                              \
        if (A##n) WBCRYPTO_gcm128_aad(&ctx,A##n,sizeof(A##n));    \
        if (C##n) WBCRYPTO_gcm128_decrypt(&ctx,C##n,out,sizeof(out));     \
        if (WBCRYPTO_gcm128_finish(&ctx,T##n,16) ||               \
...
# ifdef OPENSSL_CPUID_OBJ
    {
        ...
        WBCRYPTO_gcm128_init(&ctx, &key, (WBCRYPTO_block128_f) AES_encrypt);
        WBCRYPTO_gcm128_setiv(&ctx, IV1, sizeof(IV1));

        WBCRYPTO_gcm128_encrypt(&ctx, buf.c, buf.c, sizeof(buf));
        start = OPENSSL_rdtsc();
        WBCRYPTO_gcm128_encrypt(&ctx, buf.c, buf.c, sizeof(buf));
        gcm_t = OPENSSL_rdtsc() - start;

        WBCRYPTO_ctr128_encrypt(buf.c, buf.c, sizeof(buf),
                              &key, ctx.Yi.c, ctx.EKi.c, &ctx.mres,
                              (WBCRYPTO_block128_f) AES_encrypt);
        start = OPENSSL_rdtsc();
        WBCRYPTO_ctr128_encrypt(buf.c, buf.c, sizeof(buf),
                              &key, ctx.Yi.c, ctx.EKi.c, &ctx.mres,
                              (WBCRYPTO_block128_f) AES_encrypt);
