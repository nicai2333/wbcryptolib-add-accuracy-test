...
size_t test_wbsm4se_gcm_crypt_loop(size_t size){
    size_t count = 0;

    for (count = 0; run && count < 0xffffffffffffffff; count++)
    {
        wbcrypto_wbsm4_se_gcm_encrypt(&gcm_ctx, input, size, output);
    }
...
size_t test_wbsm4se_ecb_crypt_loop(size_t size){
    size_t count = 0;

    for (count = 0; run && count < 0xffffffffffffffff; count++)
    {
        wbcrypto_wbsm4_se_ecb_encrypt(&ctx, input, size, output);
    }
...
int main() {
    ...
    printf("\nwbsm4se_ecb:\n");
    performance_test_enc(test_wbsm4se_ecb_crypt_loop, size, 6, 3);
    printf("\nwbsm4se_cbc:\n");
    performance_test_enc(test_wbsm4se_cbc_crypt_loop, size, 6, 3);
    printf("\nwbsm4se_ctr:\n");
    performance_test_enc(test_wbsm4se_ctr_crypt_loop, size, 6, 3);
    printf("\nwbsm4se_gcm:\n");
    performance_test_enc(test_wbsm4se_gcm_crypt_loop, size, 6, 3);
